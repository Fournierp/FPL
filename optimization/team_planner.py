...
class Team_Planner:
    ...
    def get_data(self, team_id):
        ...
        self.initial_team, self.bank = get_team(team_id, self.start)
    ...
    def advanced_wildcard(self, objective_type='decay', decay_gameweek=[0.9, 0.8, 0.7], vicecap_decay=0.1, decay_bench=[0.1, 0.1, 0.1, 0.1], ft_val=0, itb_val=0):
        # Longterm Model
        model_name = 'longterm'
        ...
        # Assume a % (decay_gameweek) reliability of next week's xPts
        xp = so.expr_sum(
            (np.power(decay_gameweek[0], w - self.start) if objective_type == 'linear' else 1) *
            (
                    so.expr_sum(
                        (
                            self.starter[p, w] + self.captain[p, w] +
        ...
        itbv = so.expr_sum(
            (np.power(decay_gameweek[0], w - self.start - 1) if objective_type == 'linear' else 1) *
            (
                itb_val * self.in_the_bank[w] # Value of having 2FT
            ) for w in self.gameweeks) # Value is added to the GW when a FT is rolled so exclude the first Gw 
        ...
        # The number of players fit the requirements 2 Gk, 5 Def, 5 Mid, 3 For
        self.model.add_constraints((so.expr_sum(self.team[p, w] * self.data.loc[p, 'G'] for p in self.players) == 2 for w in self.all_gameweeks), name='gk_limit')
        self.model.add_constraints((so.expr_sum(self.team[p, w] * self.data.loc[p, 'D'] for p in self.players) == 5 for w in self.all_gameweeks), name='def_limit')
        self.model.add_constraints((so.expr_sum(self.team[p, w] * self.data.loc[p, 'M'] for p in self.players) == 5 for w in self.all_gameweeks), name='mid_limit')
        self.model.add_constraints((so.expr_sum(self.team[p, w] * self.data.loc[p, 'F'] for p in self.players) == 3 for w in self.all_gameweeks), name='for_limit')
        ...
        self.model.add_constraints((self.team[p, w] == self.team[p, w - 1] for p in self.players for w in self.gameweeks[1:]),
        ...
        random_sampled_team = np.random.choice([p for p in self.players if self.team[p, self.start].get_value()], 10, replace=False)
        ...
        # Assume a % (decay_gameweek) reliability of next week's xPts
        xp = so.expr_sum(
            (np.power(decay_gameweek[1], w - self.start) if objective_type == 'linear' else 1) *
            (
                    so.expr_sum(
                        (
                            self.starter[p, w] + self.captain[p, w] +
        ...
        ftv = so.expr_sum(
            (np.power(decay_gameweek[1], w - self.start - 1) if objective_type == 'linear' else 1) *
            (
                ft_val * self.rolling_transfers[w] # Value of having 2FT
            ) for w in self.gameweeks[1:]) # Value is added to the GW when a FT is rolled so exclude the first Gw 
        ...
        self.model.set_objective(- xp - ftv - itbv + hits_handicap, name='total_xp_obj', sense='N')

        # Initial conditions: set team and FT depending on the team
        self.model.add_constraints((self.team[p, self.start - 1] == 1 for p in self.initial_team), name='initial_team')
        self.model.add_constraints((self.team[p, self.start] == 1 for p in random_sampled_team), name='initial_wc_team')
        self.model.add_constraint(self.free_transfers[self.start - 1] == 1, name='initial_ft')
        self.model.add_constraint(self.in_the_bank[self.start - 1] == self.bank, name='initial_itb')
        ...
        self.model.add_constraints((self.hits[w] == 0 for w in self.gameweeks[1:]), name='hits_max')
        ...
        more_players = np.random.choice([p for p in self.players if (self.team[p, self.start].get_value() and p not in random_sampled_team)], 2, replace=False)
        ...
        # Assume a % (decay_gameweek) reliability of next week's xPts
        xp = so.expr_sum(
            (np.power(decay_gameweek[2], w - self.start) if objective_type == 'linear' else 1) *
            (
                    so.expr_sum(
                        (
                            self.starter[p, w] + self.captain[p, w] +
        ...
        ftv = so.expr_sum(
            (np.power(decay_gameweek[2], w - self.start - 1) if objective_type == 'linear' else 1) *
            (
                ft_val * self.rolling_transfers[w] # Value of having 2FT
            ) for w in self.gameweeks[1:]) # Value is added to the GW when a FT is rolled so exclude the first Gw 
    ...
    def automated_chips_model(self, objective_type='decay', decay_gameweek=0.9, vicecap_decay=0.1, decay_bench=[0.1, 0.1, 0.1, 0.1], ft_val=0, itb_val=0, triple_val=12, bboost_val=14, freehit_val=18, wildcard_val=18):
        ...
        self.free_transfers = self.model.add_variables(np.arange(self.start-1, self.start+self.period+1), name='ft', vartype=so.integer, lb=0)
        ...
        starter = so.expr_sum(
            (np.power(decay_gameweek, w - self.start) if objective_type == 'linear' else 1) *
            so.expr_sum(self.starter[p, w] * self.data.loc[p, f'{w}_Pts'] for p in self.players)
            for w in self.gameweeks)

        cap = so.expr_sum(
            (np.power(decay_gameweek, w - self.start) if objective_type == 'linear' else 1) *
            so.expr_sum(self.captain[p, w] * self.data.loc[p, f'{w}_Pts'] for p in self.players)
            for w in self.gameweeks)

        vice = so.expr_sum(
            (np.power(decay_gameweek, w - self.start) if objective_type == 'linear' else 1) *
            so.expr_sum(vicecap_decay * self.vicecaptain[p, w] * self.data.loc[p, f'{w}_Pts'] for p in self.players)
            for w in self.gameweeks)

        bench = so.expr_sum(
            (np.power(decay_gameweek, w - self.start) if objective_type == 'linear' else 1) *
            so.expr_sum(so.expr_sum(decay_bench[o] * self.bench[p, w, o] for o in order) * self.data.loc[p, f'{w}_Pts'] for p in self.players
            ) for w in self.gameweeks)

        txc = so.expr_sum(
            (np.power(decay_gameweek, w - self.start) if objective_type == 'linear' else 1) *
            so.expr_sum(2 * self.triple[p, w] * self.data.loc[p, f'{w}_Pts'] for p in self.players)
            for w in self.gameweeks)
        ...
        self.model.add_constraint(so.expr_sum(self.triple[p, w] for p in self.players for w in self.gameweeks) <= -- (not self.threexc_used), name='tc_once')
        self.model.add_constraint(so.expr_sum(self.bboost[w] for w in self.gameweeks) <= -- (not self.bboost_used), name='bb_once')
        self.model.add_constraint(so.expr_sum(self.freehit[w] for w in self.gameweeks) <= -- (not self.freehit_used), name='fh_once')
        self.model.add_constraint(so.expr_sum(self.wildcard[w] for w in self.gameweeks) <= -- (not self.wildcard_used), name='wc_once')
        ...
        self.model.add_constraint(so.expr_sum(so.expr_sum(self.triple[p, w] for p in self.players) + self.bboost[w] + self.freehit[w] + self.wildcard[w] for w in self.gameweeks) <= 1, name='chip_once')

        # Team
        # The number of players must fit the requirements 2 Gk, 5 Def, 5 Mid, 3 For
        self.model.add_constraints((so.expr_sum(self.team[p, w] * self.data.loc[p, 'G'] for p in self.players) == 2 for w in self.gameweeks), name='gk_limit')
        self.model.add_constraints((so.expr_sum(self.team[p, w] * self.data.loc[p, 'D'] for p in self.players) == 5 for w in self.gameweeks), name='def_limit')
        self.model.add_constraints((so.expr_sum(self.team[p, w] * self.data.loc[p, 'M'] for p in self.players) == 5 for w in self.gameweeks), name='mid_limit')
        self.model.add_constraints((so.expr_sum(self.team[p, w] * self.data.loc[p, 'F'] for p in self.players) == 3 for w in self.gameweeks), name='for_limit')
        ...
        # The number of Freehit players must fit the requirements 2 Gk, 5 Def, 5 Mid, 3 For
        self.model.add_constraints((so.expr_sum(self.team_fh[p, w] * self.data.loc[p, 'G'] for p in self.players) == 2 * self.freehit[w] for w in self.gameweeks), name='gk_limit_fh')
        self.model.add_constraints((so.expr_sum(self.team_fh[p, w] * self.data.loc[p, 'D'] for p in self.players) == 5 * self.freehit[w] for w in self.gameweeks), name='def_limit_fh')
        self.model.add_constraints((so.expr_sum(self.team_fh[p, w] * self.data.loc[p, 'M'] for p in self.players) == 5 * self.freehit[w] for w in self.gameweeks), name='mid_limit_fh')
        self.model.add_constraints((so.expr_sum(self.team_fh[p, w] * self.data.loc[p, 'F'] for p in self.players) == 3 * self.freehit[w] for w in self.gameweeks), name='for_limit_fh')

        # The number of Freehit players from a team must not exceed three
        self.model.add_constraints((so.expr_sum(self.team_fh[p, w] * self.data.loc[p, team_name] for p in self.players) <= 3 * self.freehit[w]
                            for team_name in self.team_names for w in self.gameweeks), name='team_limit_fh')

        # Starters
        # The formation must be valid i.e. Minimum one goalkeeper, 3 defenders, 2 midfielders and 1 striker on the lineup
        self.model.add_constraints((so.expr_sum(self.starter[p, w] for p in self.players) == 11 + 4 * self.bboost[w] for w in self.gameweeks), name='11_starters')
        self.model.add_constraints((so.expr_sum(self.starter[p, w] * self.data.loc[p, 'G'] for p in self.players) == 1 + self.bboost[w] for w in self.gameweeks), name='gk_min')
        self.model.add_constraints((so.expr_sum(self.starter[p, w] * self.data.loc[p, 'D'] for p in self.players) >= 3 for w in self.gameweeks), name='def_min')
        self.model.add_constraints((so.expr_sum(self.starter[p, w] * self.data.loc[p, 'M'] for p in self.players) >= 2 for w in self.gameweeks), name='mid_min')
        self.model.add_constraints((so.expr_sum(self.starter[p, w] * self.data.loc[p, 'F'] for p in self.players) >= 1 for w in self.gameweeks), name='for_min')

        # Linearization constraints to limit the Freehit Team
        self.model.add_constraints((self.starter[p, w] <= self.team_fh[p, w] + self.aux[p, w] for p in self.players for w in self.gameweeks), name='4.24')
        self.model.add_constraints((self.aux[p, w] <= self.team[p, w] for p in self.players for w in self.gameweeks), name='4.25')
        self.model.add_constraints((self.aux[p, w] <= 1 - self.freehit[w] for p in self.players for w in self.gameweeks), name='4.26')

        # Captain
        # One captain (or one triple cap) must be picked once
        self.model.add_constraints((so.expr_sum(self.captain[p, w] + self.triple[p, w] for p in self.players) == 1 for w in self.gameweeks), name='one_captain')
        # One vice captain must be picked once
        self.model.add_constraints((so.expr_sum(self.vicecaptain[p, w] for p in self.players) == 1 for w in self.gameweeks), name='one_vicecaptain')
        # The captain, vice captain and triple captain must be starters and must not be the same player
        self.model.add_constraints((self.captain[p, w] + self.triple[p, w] + self.vicecaptain[p, w] <= self.starter[p, w] for p in self.players for w in self.gameweeks), name='cap_in_starters')
        ...
        self.model.add_constraints((self.starter[p, w] + so.expr_sum(self.bench[p, w, o] for o in order) <= self.team[p, w] + 10000 * self.freehit[w] for p in self.players for w in self.gameweeks), name='bench_team')
        # The players not started in the freehit team are benched 
        self.model.add_constraints((self.starter[p, w] + so.expr_sum(self.bench[p, w, o] for o in order) <= self.team_fh[p, w] + 10000 * (1 - self.freehit[w]) for p in self.players for w in self.gameweeks), name='bench_team_fh')
        ...
        self.model.add_constraints((self.team[p, w - 1] + self.buy[p, w] - self.sell[p, w] == self.team[p, w] for p in self.players for w in self.gameweeks), name='team_similarity')
        # The player must not be sold and bought simultaneously
        self.model.add_constraints((self.sell[p, w] + self.buy[p, w] <= 1 for p in self.players for w in self.gameweeks), name='single_buy_or_sell')

        # The cost of the freehit squad must exceed the budget
        self.model.add_constraints((self.in_the_bank[w - 1] + so.expr_sum(self.team[p, w - 1] * self.data.loc[p, 'SV'] for p in self.players) >= so.expr_sum(self.team_fh[p, w] * self.data.loc[p, 'BV'] for p in self.players) for w in self.gameweeks), name='budget_fh')
        # On Freehit GW the number of transfers must be zero
        self.model.add_constraints((so.expr_sum(self.sell[p, w] for p in self.players) <= 15 * (1 - self.freehit[w]) for w in self.gameweeks), name='zero_sold_fh')
        self.model.add_constraints((so.expr_sum(self.buy[p, w] for p in self.players) <= 15 * (1 - self.freehit[w]) for w in self.gameweeks), name='zero_bought_fh')
        
        # The number of players bought and sold is equal
        self.model.add_constraints((so.expr_sum(self.buy[p, w] for p in self.players) == so.expr_sum(self.sell[p, w] for p in self.players) for w in self.gameweeks), name='equal_transfers')

        # Transfers
        # The rolling transfer must be equal to the number of free transfers not used (+ 1)
        self.model.add_constraints((15 * self.wildcard[w] + self.free_transfers[w] - so.expr_sum(self.buy[p, w] for p in self.players) + 1 + self.hits[w] >= self.free_transfers[w + 1] for w in self.gameweeks), name='rolling_ft_rel')        #  
        # The hits value is zero only when the number of FT is 2
        self.model.add_constraints((10000 * (2 - self.free_transfers[w + 1]) >= self.hits[w] for w in self.gameweeks), name='4.42')
        ...
        self.model.add_constraints((self.free_transfers[w + 1] <= 2 + self.wildcard[w] + self.freehit[w] for w in self.gameweeks), name='max_ft')
