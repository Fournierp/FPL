...
class Understat:
    ...
    def save_match_data(self, output_file, shots, player):
        ...
        player_df = player_df.append(pd.DataFrame.from_dict(player['a'], orient='index'))
        ...
        for i in range(self.last_match_id, self.last_match_id + 500):
            ...
            for league in desc:
                if league.find(text=re.compile(league_name)):
                    ...
                    team_home = parsed_html.findAll("div", class_="roster roster-home")
                    for team in team_home:
                        team_h = team.text.strip().partition('\n')[0]
                    
                    team_away = parsed_html.findAll("div", class_="roster roster-away")
                    for team in team_away:
                        team_a = team.text.strip().partition('\n')[0]
                        
                    self.save_match_data(self.root + f'/matches/{league.text.strip()[-11:]}_{team_h}_vs_{team_a}', shots, player)

                if (
                    league.find(text=re.compile(league_name)) or
    ...
    def get_fpl_to_understat_ids(self):
        ...
        self.players['id'] = self.players.apply(lambda row : self._lambda_req(row), axis = 1)
    ...
    def get_player_data(self, history=False):
        ...
        for season in seasons:
            ...
            for idx in player_ids:
                ...
                self.save_player_data(self.root + f'/players/{name}', groups, shots, matches)
