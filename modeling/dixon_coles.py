...
class Dixon_Coles:
    ...
    with importance weight given to recent games and adjustments to low scoring
    games
    """
    ...
    def __init__(self, games):
        ...
        self.games = self.games.loc[:, [
            "score1", "score2", "team1", "team2", "weight"]]
    ...
    def _low_score_adj(self, df):
        ...
        Args:
            df (pd.DataFrame): Df with infered and actual scores

        Returns:
            pd.DataFrame: Df with adjusted scores
        """
        return np.select(
            [
    ...
    def fit(self, parameters, games):
        ...
        aggregate_df["score1_loglikelihood"] = poisson.logpmf(
            aggregate_df["score1"], aggregate_df["score1_infered"])
        aggregate_df["score2_loglikelihood"] = poisson.logpmf(
    ...
    def optimize(self):
        ...
        # Set the home rating to have unique values for reproducibility
        constraints = [{
            "type": "eq",
            "fun": lambda x:
        ...
        # Set the maximum and minimum values for the parameters
    ...
    def score_mtx(self, team1, team2, max_goals=8):
        ...
        Args:
            team1 (int): Home team name
            team2 (int): Away team name

        Returns:
            (array): Odds of each score
        """
        ...
        for gw in range(1, 39):
            ...
            if gw in self.test_games['event'].values:
                ...
                rps_list.append(
                    self.evaluate(
                        self.test_games[
                            self.test_games['event'] == gw])['rps'].values)
...
if __name__ == "__main__":
    ...
    if os.path.isfile("data/predictions/scores/dixon_coles.csv"):
        past_predictions = pd.read_csv(
            "data/predictions/scores/dixon_coles.csv")
        (
            pd.concat(
                [
                    past_predictions,
