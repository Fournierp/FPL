...
class Poisson_Time_Decay:
    ...
    with importance weight given to recent games """
    ...
    def fit(self, parameters, games):
        ...
        aggregate_df["loglikelihood"] = (
            ...
            aggregate_df["score2_loglikelihood"]) * aggregate_df['weight']
    ...
    def optimize(self):
        ...
        # Set the maximum and minimum values of the parameters
...
if __name__ == "__main__":
    ...
    season_games = (
        pd.merge(
            season_games, fixtures,
            left_on='kickoff_time', right_on='kickoff_time')
        .drop_duplicates()
        )
    ...
    if os.path.isfile("data/predictions/scores/poisson_decay.csv"):
        past_predictions = pd.read_csv(
            "data/predictions/scores/poisson_decay.csv")
        (
            pd.concat(
                [
                    past_predictions,
